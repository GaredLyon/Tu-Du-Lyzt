...
export const Main = ({asideVisible, toggleAside}) => {
  ...
  return (
    <div className='container-columns' data-theme>
      ...
      {groups && (
        <>
          <Column title='Pendiente' color='gray' key={`column${1}`} group={groups[0]}/>
          <Column title='Proceso'  color='purple' key={`column${2}`} group={groups[1]}/>
          <Column title='Completados' color='green' key={`column${3}`} group={groups[2]}/>
        </>
      )}
